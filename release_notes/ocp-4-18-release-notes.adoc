...
[id="ocp-release-notes-rhcos-rhel-9.6-packages_{context}"]
===== {op-system} now uses {op-system-base} 9.6

{op-system} now uses {op-system-base-first} 9.6 packages in {product-title} {product-version}. These packages ensure that your {product-title} instances receive the latest fixes, features, enhancements, hardware support, and driver updates. As an Extended Update Support (EUS) release, {product-title} 4.16 is excluded from this change and will continue to use {op-system-base} 9.4 EUS packages for the entirety of its lifecycle.
...
Previously, the `/etc/iscsi/initiatorname.iscsi` file was present by default on {op-system} images. With this release, if the file was not already created, the `initiatorname.iscsi` file is created at run time when the `iscsid.service` and subsequent `iscsi-init.service` services start. This service is not enabled by default and might affect any CSI drivers that rely on reading the contents of the `initiatorname.iscsi` file prior to starting the service.
